    ...
    pub fn calculate_base_fee(
        ...
        if remaining_lamports >= pool.min_sol_for_min_fee {
            ...
            if current_sol_vault_lamports >= pool.min_sol_for_min_fee {
                let amount_above_min_fee_threshold =
                    current_sol_vault_lamports - pool.min_sol_for_min_fee;
                let amount_below_min_fee_threshold =
                    unstake_lamports - amount_above_min_fee_threshold;

                Ok((pool.fee_min as u64)
                    .checked_mul(amount_above_min_fee_threshold)
                    .ok_or(LiquidUnstakerErrorCode::MathOverflow)?
                    .checked_add(
                        Fee::calculate_linear_base_fee(
